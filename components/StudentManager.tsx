...
import { studentApi, getDepartments } from '../services/api.ts';
...
const StudentManager: React.FC = () => {
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        try {
            await studentApi.add(optimisticStudent);
            addToast(`Student ${optimisticStudent.name} added. They will be prompted to set a password on first login.`, "success");
            loadData();
        } catch (err: any) {
    ...
    const handlePreview = (student: Student) => {
        setStudentToPreview({ id: student.id, name: student.name, role: 'student' });
    };
    
    const handleAnalyze = (student: Student) => {
        setStudentToAnalyze(student);
    };
    ...
    return (
        <div className="space-y-4">
            ...
            <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end p-4 bg-slate-50 dark:bg-slate-900/50 border dark:border-slate-700 rounded-lg">
                ...
                <div>
                    <button type="submit" className="w-full justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 active:scale-95 transition-transform">Add Student</button>
                </div>
            </form>

            <div className="relative">
                <MagnifyingGlassIcon className="pointer-events-none absolute top-3.5 left-4 h-5 w-5 text-slate-400" />
                <input type="text" placeholder="Search students..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="block w-full rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 py-2.5 pl-11 pr-5 text-slate-900 dark:text-slate-200 placeholder:text-slate-400 sm:text-sm" />
            </div>
                        ...
                        {filteredStudents.map((student, i) => (
                            <motion.li key={student.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2, delay: i * 0.05 }} className="flex justify-between items-center p-2 bg-white dark:bg-slate-700/50 border dark:border-slate-200 dark:border-slate-700 rounded-md">
                                ...
                                <div className="flex items-center gap-1">
                                    <button onClick={() => handleAnalyze(student)} className="p-1.5 text-slate-400 hover:text-purple-600 hover:bg-purple-100 dark:hover:bg-purple-900/50 rounded-full" title="Analyze Workload">
                                        <SparklesIcon className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handlePreview(student)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 rounded-full" title="Preview Schedule">
                                        <EyeIcon className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handleDelete(student)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full" title="Delete Student">
