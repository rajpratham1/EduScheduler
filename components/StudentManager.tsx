...
const StudentManager: React.FC = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            const [students, depts] = await Promise.all([studentApi.getAll(), api.getDepartments()]);
            setStudentList(students);
            setDepartments(depts);
            if (depts.length > 0 && !selectedDepartmentId) setSelectedDepartmentId(depts[0].id);
        } catch (err) {
    ...
    const filteredStudents = useMemo(() => studentList.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase())), [studentList, searchTerm]);

    const handleAdd = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!newStudentName || !newStudentEmail || !selectedDepartmentId) return;
        try {
            const addedStudent = await studentApi.add({ name: newStudentName, email: newStudentEmail, department_id: Number(selectedDepartmentId) });
            setStudentList(prev => [...prev, addedStudent]);
    ...
    return (
        <div className="space-y-4">
            ...
            <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end p-4 bg-slate-50 dark:bg-slate-900/50 border dark:border-slate-700 rounded-lg">
                <div className="sm:col-span-2"><label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Name</label><input value={newStudentName} onChange={e => setNewStudentName(e.target.value)} className="mt-1 block w-full border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-md shadow-sm text-sm dark:text-slate-200" required /></div>
                ...
                <div className="sm:col-span-2"><button type="submit" className="w-full sm:w-auto px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 active:scale-95 transition-transform">Add Student</button></div>
            </form>
            ...
            <div className="max-h-60 overflow-y-auto pr-2">{isLoading ? <SkeletonLoader /> : <ul className="space-y-2"><AnimatePresence>{filteredStudents.length > 0 ? filteredStudents.map((student, i) => (<motion.li key={student.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2, delay: i * 0.05 }} className="flex justify-between items-center p-2 bg-white dark:bg-slate-700/50 border dark:border-slate-200 dark:border-slate-700 rounded-md"><div><p className="font-medium text-sm text-slate-800 dark:text-slate-200">{student.name}</p><p className="text-xs text-slate-500 dark:text-slate-400">{departments.find(d => d.id === student.department_id)?.name}</p></div><div className="flex items-center gap-1"><button onClick={() => setStudentToPreview({ ...student, role: 'student' })} className="p-1.5 text-slate-400 hover:text-indigo-600 rounded-full hover:bg-indigo-100 dark:hover:bg-indigo-900/50" title="Preview Schedule"><EyeIcon className="w-4 h-4" /></button><button onClick={() => setStudentToDelete(student)} className="p-1.5 text-slate-400 hover:text-red-600 rounded-full hover:bg-red-100 dark:hover:bg-red-900/50" title="Delete Student"><TrashIcon className="w-4 h-4" /></button></div></motion.li>)) : (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-8"><UserGroupIcon className="w-10 h-10 mx-auto text-slate-400" /><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">No students found.</p></motion.div>)}</AnimatePresence></ul>}</div>
            ...
            <WorkloadAnalysisModal student={studentToAnalyze} onClose={() => setStudentToAnalyze(null)} />
        </div>
    );
};
