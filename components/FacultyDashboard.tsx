...
const FacultyDashboard: React.FC<FacultyDashboardProps> = ({ user }) => {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const facultyScheduleData = publishedSchedule.filter(s => s.faculty_id === user.id);
            const hydrated = hydrateSchedule(facultyScheduleData, subjects, faculty, classrooms);
    ...
    if (isLoading) {
        return <div className="p-4">Loading your teaching schedule...</div>;
    }

    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Your Teaching Schedule</h2>
            {schedule.length > 0 ? (
                ...
                <div className="text-center py-12 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-lg">
                    <p className="text-slate-500 dark:text-slate-400">You have not been assigned any classes in the current schedule.</p>
                </div>
            )}
        </div>
    );
