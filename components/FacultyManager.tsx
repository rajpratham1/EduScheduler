...
import { facultyApi, getDepartments } from '../services/api.ts';
...
const FacultyManager: React.FC = () => {
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        try {
            await facultyApi.add(optimisticFaculty);
            addToast(`Faculty member ${optimisticFaculty.name} added. They will be prompted to set a password on first login.`, "success");
            loadData();
        } catch (err: any) {
    ...
    const handlePreview = (faculty: Faculty) => {
        setFacultyToPreview({ id: faculty.id, name: faculty.name, role: 'faculty' });
    };
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="relative">
                ...
                <input type="text" placeholder="Search faculty..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="block w-full rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 py-2.5 pl-11 pr-5 text-slate-900 dark:text-slate-200 placeholder:text-slate-400 sm:text-sm" />
            </div>
            ...
            <div className="max-h-60 overflow-y-auto pr-2">
                        ...
                        {filteredFaculty.map((faculty, i) => (
                            <motion.li key={faculty.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2, delay: i * 0.05 }} className="flex justify-between items-center p-2 bg-white dark:bg-slate-700/50 border dark:border-slate-200 dark:border-slate-700 rounded-md">
                                ...
                                <div className="flex items-center gap-1">
                                    <button onClick={() => handlePreview(faculty)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 rounded-full">
             ...
             <ConfirmationModal isOpen={!!facultyToDelete} onClose={() => setFacultyToDelete(null)} onConfirm={executeDelete} title="Delete Faculty Member" message={`Are you sure you want to delete ${facultyToDelete?.name}?`} />
